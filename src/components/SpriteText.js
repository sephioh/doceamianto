    ...
    init: function() {
        this.bind("Change", function(obj) {
            ...
            if (tileSize && this._text) {
                ...
                if (this._align === "center") startx += (this._w - textwidth) / 2;
                if (this._align === "right") startx += (this._w - textwidth);
                for (i in txt) {
                    l = txt[i];
                    ...
                    e = Crafty.e("2D, " + type + ", " + this.charName(this._font, ch)) // create entity for the letter
                            .attr({x: posx, y: this._y, z: this._z, w: tileSize, h: tileSize});
